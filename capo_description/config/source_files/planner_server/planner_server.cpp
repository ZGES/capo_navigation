...
PlannerServer::on_configure(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  duration_publisher_ = create_publisher<builtin_interfaces::msg::Duration>("duration", 1);
...
PlannerServer::on_activate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  duration_publisher_->on_activate();
...
PlannerServer::on_deactivate(const rclcpp_lifecycle::State & /*state*/)
{
  ...
  duration_publisher_->on_deactivate();
...
PlannerServer::computePlan()
{
  ...
  try {
    ...
    publishDuration(result->planning_time);
...
void
PlannerServer::publishDuration(const builtin_interfaces::msg::Duration & duration)
{
  auto msg = std::make_unique<builtin_interfaces::msg::Duration>(duration);
  if (
    duration_publisher_->is_activated() &&
    this->count_subscribers(duration_publisher_->get_topic_name()) > 0)
  {
    duration_publisher_->publish(std::move(msg));
  }
}
